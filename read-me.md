...
2 -> PyAeroCounter.exe default input is to read a PDF document named 'pdffile.pdf'. 
...
3 -> The user must have the following software installed:
...
4 -> For advanced users that want to run the source code, it is necessary to install python and the following packages:
...
5 -> For advanced users that want to compile the source code, use pyinstaller.
Get it: https://pypi.org/project/pyinstaller/

pyinstaller --onefile --nowindowed PyAeroCounter.py

6 -> If you have any problem or suggestion, please send it to www.aeroct.com.br